...
const loadCountries = async (searchTerm = '') => {
    ...
    if (filteredCountries.length > 0) {
        filteredCountries.forEach(country => {
            const flagUrl = country.flag || `images/Flag_of_Vatican_City.svg`; // Fallback if flag URL is missing
            cardContainer.innerHTML += `
        <div class="col-12 col-sm-12 col-md-6 col-lg-3">
            <div class="card rounded-5 my-2">
                <a href="view.html?countryname=${country.name}" style="text-decoration: none; color: inherit;">
                    <div class="imgcontainer">
                        <img src="${flagUrl}" alt="${country.name}" onerror="this.onerror=null;this.src='images/Flag_of_Vatican_City.svg';">
                    </div>
